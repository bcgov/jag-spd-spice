...
public partial class OnPremSharePointFileManager : ISharePointFileManager
{
    ...
    /// This method attempts to retrieve a well-known document library to verify connectivity.
    /// </summary>
    ...
    public async Task<bool> TestStatus()
    {
        ...
        {
            _logger.LogWarning(
                "[OnPremSharePointFileManager] TestStatus - SharePoint is not configured (IsValid=false)"
            );
            return false;
        }
        ...
        // Use the "Account" document library as a well-known library for health checks
        string listTitle = SharePointConstants.AccountFolderDisplayName;
        string title = Uri.EscapeUriString(listTitle);
        string query = $"web/lists/GetByTitle('{title}')";
        ...
        {
            _logger.LogDebug(
                "[OnPremSharePointFileManager] TestStatus - Testing SharePoint connection to '{ListTitle}'",
                listTitle
            );
            ...
            // We're only interested in the status of the response
            if (statusCode == HttpStatusCode.OK)
            {
                result = true;
                _logger.LogInformation(
                    "[OnPremSharePointFileManager] TestStatus - SharePoint is healthy (status: {StatusCode})",
                    statusCode
                );
            }
            else
            {
                _logger.LogWarning(
                    "[OnPremSharePointFileManager] TestStatus - SharePoint returned non-OK status: {StatusCode}",
                    statusCode
                );
            }
        }
        catch (Exception ex)
        {
            ...
            _logger.LogError(
                ex,
                "[OnPremSharePointFileManager] TestStatus - Exception occurred while testing SharePoint: {ErrorMessage}",
                ex.Message
            );
            result = false;
        }
