...
public partial class CloudSharePointFileManager : ISharePointFileManager
{
    ...
    private async Task EnsureSiteIdResolvedAsync()
    {
        ...
        if (string.IsNullOrEmpty(SiteId))
        {
            ...
            throw new InvalidOperationException($"Failed to resolve site ID for {SiteUrl}");
        }
    ...
    public async Task CreateFolder(string listTitle, string folderName)
    {
        ...
        if (string.IsNullOrEmpty(listId))
        {
            throw new InvalidOperationException($"Document library '{listTitle}' not found");
        }
    ...
    )
    {
        ...
        {
            throw new InvalidOperationException("Failed to create upload session - no uploadUrl returned");
        }
    ...
    public async Task<byte[]> DownloadFile(string serverRelativeUrl)
    {
        ...
        if (string.IsNullOrEmpty(listId))
        {
            ...
            throw new FileNotFoundException($"Document library '{listTitle}' not found");
        }
